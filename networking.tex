...
%in realtà, siccome l'USB si usa molto, si potrebbe anche approfondire (le slides però dicono poco), lo faccio? 
...
\end{itemize}
\image[Bridge-net.png][scale=0.5]{ciascun segmento di rete, collegato ad una porta di un bridge, costituisce un \textit{dominio di collisione} separato}
...
\para{Funzionamento} la funzione di instradamento è implementata mantenendo in un \textbf{buffer locale} tutti gli \textit{indirizzi MAC} degli host connessi alla rete, suddivisi per \textit{porta dello switch}.\\
...
\image[Switched-net.png][scale=0.5]{lo switch riduce la rete in un insieme di \textit{domini di collisione} di dimensioni minime (al limite costituiti ognuno da un singolo host)}
...
.....\\
Esistono diversi modi per proteggere una rete, come l'oscuramento, l'hardening e il firewalling.
\subsub{Oscuramento}
...
\paragraph{Monitoraggio} prevede l'uso dei comandi UNIX:
\begin{itemize}
\item \textbf{ps -aux} per controllare i processi attivi nel sistema e chi li ha generati
\item \textbf{who} per vedere chi è collegato al sistema
\item \textbf{ls -lR} per creare periodicamente una lista sulle informazioni di ogni file di sistema e confrontarla (con \code{diff}) con quella precedente
...
Le richieste verso l'host su cui gira un wrapper TCP vengono elaborate da un servizio di autenticazione, che verifica che l'indirizzo del chiamante sia incluso nell'elenco di \textit{/etc/hosts.allow}: se è così permette l'accesso, altrimenti verifica non sia incluso in \textit{/etc/hosts.deny}.\\
In entrambi files è ache possibile definire delle \textbf{regole di filtraggio}, che hanno effetto \textit{dal basso verso l'alto} e quelle di hosts.allow hanno la precedenza su quelle di hosts.deny.\\
Il formato delle regole è:\\
\code{<elenco-servizi> : <elenco-client> [: spawn <comando-shell>]}\\
E le seguenti keyworks sono uste per specificare host o gruppi di host: ALL \textit{(tutti i client/servizi)}, LOCAL \textit{(tutti gli host locali)}, KNOWN \textit{(host riconosciuti dal sistema)}, UNKNOWN \textit{(host non riconosciuti)}, PARANOID \textit{(host il cui nome non corrisponde all'indirizzo)}.\\
In oltre \code{spawn} permette di eseguire normali comandi da shell qualora una richiesta entrante soddisfa una regola. Esempio in hosts.deny: \\ \code{\# rifiuta *.example.com e scrive in connections.log alcune info}\\ \code{ALL : .example.com : spawn (/bin/echo \\ \%a from \%h attempted to access \%p >> /var/log/connections.log)}\\
...
\item \textbf{Funzioni}: Permette di impostare per il server una configurazione \textit{firewall-like}, attraverso due funzioni definibili per ogni servizio:  \textbf{bind} (collega un servizio ad un IP, ossia accetta le richieste per il servizio inoltrate solo a quell'IP) e \textbf{redirect} (le richieste per questo servizio saranno redirette verso l'hostname o IP specificato come parametro)
...
Per ogni pacchetto ricevuto/inviato viene analizzata l'intestazione dei pacchetti (Header IP, Header TCP/UDP) e sottoposta a delle regole contenute in \textit{tabelle}, e se ne soddisfa una decide che \textit{azione prendere}.
\begin{itemize}
\item Le regole di filtraggio sono definite mediante \textbf{TABLES} (lista di regole) 
\begin{itemize}
\item \textit{filter} (regola il firewalling)
\item \textit{nat} (regola le attività di natting)
\item \textit{mangle} (interviene sull'alterazione dei pacchetti)
\end{itemize}
ad ognuna delle quali vengono collegate le \textbf{CHAINS} (operazioni da fare sul pacchetto)
\begin{itemize}
\item \textit{INPUT} riguarda i pacchetti destinati al sistema
\item \textit{OUTPUT} riguarda i pacchetti originati dal sistema e destinati ad uscire
\item \textit{FORWARD} riguarda i pacchetti che attraversano il sistema, con IP e destinazione esterni.
\end{itemize}
\item Le azioni da prendere sono \textbf{allow} se lo lascia passare, \textbf{deny} se lo blocca e lo rimanda al mittente o \textbf{drop} se lo blocca e lo scarta
\end{itemize}
In Linux, \textit{\textbf{NetFilter}} è il filtro di pacchetti  implementato nel kernel, la cui interfaccia è rappresentata da \code{iptables} (che sostituisce l'\code{ipchains} dei vecchi kernel). La differenza sostanziale è che \textit{ipchains} è uno \textbf{stateless firewall} (filtra i pacchetti controllando i flag SYN, senza considerare il contesto di quelli ricevuti prima), mentre \textit{iptables} è uno \textbf{stateful firewall} (tiene traccia delle connessioni mediante tabelle di richieste precedenti)
\begin{itemize}
\item NetFilter è quindi in grado di stabilire il \textbf{contesto} del pacchetto: \textbf{NEW} (appartiene ad una nuova connessione), \textbf{ESTABLISHED} (ad una connessione esistente), \textbf{RELATED} (legato ad una connessione esistente) o \textbf{INVALID} (pacchetto sospetto, non legato ad alcuna connessione stabilita).
\end{itemize}
...
\paragraph{Architetture} è possibile configurare un firewall in diverse modalità:
\begin{itemize}
