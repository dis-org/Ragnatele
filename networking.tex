...
\end{itemize} 
\para{ASN} Ogni AS è identificato da un ASN(Autonompus System Number) assegnato dall'\vedi{ICANN} ai RIR(Regional Internet Registers) seguendo le norme definite nell'\vedi{RFC}1930. 
\subsub{Router}
...
\item \textbf{Routing Statico}: utilizzato quasi solamente per gli host, prevede la definizione manuale delle varie \italic{route}. In Unix-Linux le route possono essere aggiunte usando il comando \code{route add -net <indirizzo di rete> netmask <netmask x.y.z.q> gw <indirizzo gateway> }. I comandi possono essere salvati in un file di configuarzione eseguito all'avvio della macchina (In Unix-Linux \code{/etc/init.d/rc.local}) 
...
\item Reliability: affidabilità
\item Delay: ritardo
\item Bandwidth: larghezza della banda
\item Load: carico
\item Communication cost: costo della comunicazione
\end{itemize}

\sub{Famiglie di protocolli di routing}
\subsub{IGP}
Interior Gateway Protocol: famiglia di protocolli per il routing interno ad un \vedi{AS}, iclude \vedi{RIP}, IGRP e \vedi{OSPF}.
\subsub{EGP}
Exterior Gateway Protocol: famiglia di protocolli per il routing tra due diversi \vedi{AS}, include EGP (storico), \vedi{BGP} e IDRP (OSI Inter-Domain Routing Protocol). 
\subsub{CIDR}
Classles Inter-Domain Routing: include \vedi{OSPF}, \vedi{BGP} e IS-IS.
