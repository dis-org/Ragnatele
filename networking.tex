...
Simple Network Management Protocol: protocollo per la gestione di reti internet che permette di monitorare e configurare, attraverso \vedi{UDP} sulle porte $161$ e $162$, i vari dispositivi di rete. \\Ne esistono più versioni, le più importanti sono:
\begin{itemize}
...
\end{itemize}
L'SNMP introduce i seguenti concetti:
\begin{itemize}
\item \italic{Agent}: qualsiasi dispositivo di rete;
\item \italic{Manager}: stazione che gira un programma per la gestione degli \textit{agent}.
\item \vedi{MIB} (\italic{Management Information Base}): archivio delle informazioni ricevute dagli \italic{agent}, dette oggetti.
\item Protocollo di gestione (SNMP): regola le interazioni tra il \textit{manager} e gli \textit{agent}.
\item \vedi{SMI}: struttura delle informazioni.
\end{itemize}
\para{MIB} Da non confondere con il film Man in Black.
\para{SMI} Stucture of Management Information.
\subsub{NAT}
