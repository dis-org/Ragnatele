...
\subsub{FR}
Frame Relay: tecnologia di comunicazione ad alte prestazioni  utilizzata per connettere più reti, è il risultato ottenuto nel 1990 da un consorzio appositamente creato (il Frame Relay Forum), cui appartengo, tra gli altri, Cisco, Digital, Northern Telecom e Stratacom. In sintesi, FR va inteso come un modo per inviare informazioni attraverso una \vedi{WAN}, suddividendole in \textit{frames}. Utilizza una forma di \vedi{Packet switching} che ben si presta ad essere usata nelle \vedi{LAN} e in \vedi{Internet}. In una rete FR, gli endpoint sono connessi da un \textit{logical path}, denominato \textit{virtual circuit}
