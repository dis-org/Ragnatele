...
\newpage\subsub{DNS}
Il DNS (Domain Name System) è un servizio utilizzato per associare i nomi degli host (serie di parole separate da punti) ai relativi indirizzi IP (numeri separati da punti); \\ \textbf{Vantaggi: } I primi sono facili da ricordare per l'utente ma non utilizzabili dal computer, mentre con gli ultimi accade l'esatto opposto. \\In oltre è così possibile attribuire più nomi allo stesso indirizzo IP (o viceversa) per rappresentare servizi diversi forniti da uno stesso host (o più host che forniscono lo stesso servizio).\\
Il DNS prevede:
\begin{itemize}
\item \textbf{Spazio dei nomi gerarchico} : basato su un insieme di database distribuiti (garantisce l'aggiornamento a tutta la rete). L'insieme dei nomi viene suddiviso in zone dette domini che possono coprire più host ed essere suddivisi in sotto-domini e così via, formando una struttura ad albero in cui i nodi rappresentano i nomi e:
\begin{itemize}
\item I domini di primo livello (\textit{Top Level Domain - \textbf{TLD}}) sono i figli del nodo radice "." e possono essere \textbf{gTLD} (\textit{generic TLD} - .com, .edu), \textbf{ccTLD}(\textit{country-code TLD} - .it, .fr) o \textbf{infrastrutturali} (usati per la risoluzione inversa dei nomi - .arpa). Sono definiti da ICANN (\href{http://newgtlds.icann.org/en/announcements-andmedia/video/applicants}{QUI} è possibile visionare gli ultimi TLD proposti)
\item I \textit{domini di secondo livello}, in genere, appartengono alle organizzazioni che li hanno registrati e comprendono il loro e il dominio precedente separati da un punto (es. unipg.it).
...
\boximage[dns-hierarchy.png][scale=0.8]{Spazio gerarchico dei nomi}
\end{itemize}
...
\item \textbf{Server DNS} : per rendere disponibile lo spazio dei nomi e rispondere alle richieste del resolver risolvendo i nomi (name server)
\item \textbf{Client DNS} : una libreria di funzioni per generare e inviare le richieste sui nomi, interrogando i server DNS (resolver)
\end{itemize}
...
Per associare un nome a un indirizzo IP un client chiama il risolutore, passandone come parametro il nome. \\Il risolutore invia un pacchetto UDB a un server DNS locale (\textit{primary server}) che cerca il nome e se è presente nella sua cache lo restituisce; in caso contrario interroga ricorsivamente i server partendo da un root server del TLD fino ad arrivare ai server autorevoli del nome richiesto (\textit{authoritative server}), i quali invieranno la loro risposta . Ora che l'applicativo conosce l'indirizzo IP, potrà stabilire una connessione TCP con il destinatario o inviargli pacchetti UDP.
...
\begin{itemize}
\item \textbf{caching-only} reindirizza ogni richiesta del resolver ad altri server e memorizza il risultato che ritornano in una cache locale
\item \textbf{authoritative} contiene info su tutta la zona di sua competenza. \\Può essere \textbf{primary} (legge informazioni dagli zone files e gestisce le info riguardanti specifici domini), \textbf{secondary} (scarica i zone files dal primary server e li memorizza localmente nei \textit{zone file transfer}).\\
\end{itemize}
Configurazione:
\begin{itemize}
\item \textbf{Resolver (/etc/resolv.conf)}: contiene istruzioni per l'esecuzione delle richieste; Si può usare la configurazione di default, altrimenti è necessario specificare:
\begin{itemize}
...
\item \textbf{domain \textless name\textgreater}: nome del dominio di default che verrà concatenato dopo qualsiasi nome host richiesto che non contiene il carattere punto. Se dopo aver fatto richiesta il resolver fallisce, richiede l'IP dell'host concatenato a un dominio di livello superiore \textit{(es. domain dipmat.unipg.it, richiedo cartesio e il resolver chiede l'IP di cartesio.dipmat.unipg.it. In caso di fallimento, lo richiede di cartesio.unipg.it e così a salire)}.
...
\end{itemize}
\item \textbf{Named}: è necessario configurare più files: 
\begin{itemize}
...
\boximage[slide-308.PNG][scale=0.6]{Configurazione primary server}
\item secondary server: supponendo che il dominio sia \textit{unipg.it} e il primary server \textit{moe}
\boximage[slide-309.PNG][scale=0.6]{Configurazione primary server}
\end{itemize}
...
\boximage[slide-3011.PNG][scale=0.6]{Configurazione named.local}
\item \textbf{/etc/named.hosts}: zone file per la risoluzione diretta
\boximage[slide-3012.PNG][scale=0.6]{Configurazione named.host}
\item \textbf{/etc/named.rev}: zone file per la risoluzione inversa
\boximage[slide-3013.PNG][scale=0.6]{Configurazione named.rev}
\end{itemize}
\item \textbf{Resource Record}: il formato di un RR del DNS è\\
\code{[name] [ttl] IN type data}\\
con:
\begin{itemize}
\item \textbf{name}
...
Il DNS(\textit{Domain Name System}) è un servizio utilizzato per associare i nomi degli host (serie di parole separate da punti) ai relativi indirizzi \vedi{IP} (numeri separati da punti); \\ \textbf{Vantaggi: } I primi sono facili da ricordare per l'utente ma non utilizzabili dal computer, mentre con gli ultimi accade l'esatto opposto. \\In oltre è così possibile attribuire più nomi allo stesso indirizzo IP (o viceversa) per rappresentare servizi diversi forniti da uno stesso host (o più host che forniscono lo stesso servizio).\\
...
\paragraph{Composizione DNS} Il DNS prevede:
...
\item I domini di primo livello (\textit{Top Level Domain - \textbf{TLD}}) sono i figli del nodo radice "." e possono essere \\\textbf{gTLD} (\textit{generic TLD} - .com, .edu), \\\textbf{ccTLD}(\textit{country-code TLD} - .it, .fr) \\\textbf{infrastrutturali} (usati per la risoluzione inversa dei nomi - .arpa). \\Sono definiti da ICANN (\href{http://newgtlds.icann.org/en/announcements-andmedia/video/applicants}{QUI} è possibile visionare gli ultimi TLD proposti).\\

In oltre ogni regione geografica ed ogni organizzazione hanno una propria autorità responsabile del proprio TDL (per l'Italia l'IIT CNR), la quale delega la gestione tecnico-amministrativa dei vari sottodomini agli enti che ne fanno richiesta, e così via per i sottodomini dei livelli successivi.
