...
\item \textbf{Routing Statico}: adatto a \vedi{LAN} di piccole dimensioni, prevede la definizione manuale delle varie \italic{route}. In Unix-Linux le route possono essere aggiunte usando il comando \code{route add -net <indirizzo di rete> netmask <netmask x.y.z.q> gw <indirizzo gateway> }
\item \textbf{Routing Dinamico}: utilizza \vedi{Protocolli di routing}
\end{itemize}
