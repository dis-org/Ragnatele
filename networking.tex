...
\para{Nameserver}
I nameserver, o più precisamente i DNS (\textit{Domain Name Server}), sono distribuiti capillarmente nella rete e contengono una porzione delle associazioni indirizzo-nome. Va notato che indirizzi e nomi sono ovviamente intercambiabili, ma prima di effettuare una connessione, i nomi vengono convertiti in indirizzi (\textit{risoluzione nome-indirizzo IP}). Questo processo avviene nei client tramite la consultazione della \textit{host table}, che può essere configurata staticamente o aggiornata in modo dinamico per l'appunto consultando un nameserver. In quest'ultimo caso sarà sufficiente indicare in un file di configurazione i nameserver di riferimento. 
