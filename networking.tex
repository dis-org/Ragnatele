...
\para{Nome di dominio} un nome di dominio è costituito da una serie di stringhe separate da punti, la gerarchia procede da destra verso sinistra. La prima stringa da destra è detta pertanto TLD (\italic{Top Level Domain}), la cui lista completa è disponibile all'indirizzo \url{http://data.iana.org/TLD/tlds-alpha-by-domain.txt}.
\image[dns.png][scale=0.35]{Gerarchia dei domini}

\ \\I nameserver, talvolta detti anche a loro volta DNS (\textit{Domain Name Server}). sono distribuiti capillarmente nella rete e contengono una porzione delle associazioni indirizzo-nome.\\\\
L'implementazione più comune del DNS su ambiente Unix è il \italic{Berkeley Internet Name Domain} (BIND) 
