...
\begin{itemize}
\item \textbf{gTLD} (\textit{generic TLD}), ad esempio .com, .edu\footnote{gTLD riservati: \code{.example}, \code{.invalid}, \code{.localhost}, \code{.test}}...
...
\end{itemize}
Essi sono assegnati da \vedi{ICANN} alle organizzioni o alle autorità responsabili locali (per l'Italia l'IIT CNR). La lista completa è disponibile all'indirizzo \url{www.iana.org/domains/root/db}.
\item I domini di secondo livello, in genere, appartengono alle organizzazioni che li hanno registrati e comprendono il loro nome e il dominio di primo livello separati da un punto (es. unipg.it).
...
Ad ogni dominio è associato un Resource Record (RR), file ASCII che contiene record del database DNS.  %\vedi resource record
\paragraph{Name server} Server DNS che risponde alle richieste del resolver, effettuando le conversioni nomi/indirizzi.
\paragraph{Resolver} Libreria di funzioni presente nei client per generare e inviare le query relative ai nameserver.
...
\end{itemize}
\end{itemize}
Di seguito, le configurazioni del resolver, del named e degli \textbf{Zone files} di BIND:
